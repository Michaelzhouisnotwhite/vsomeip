...
class service_sample {
 public:
  explicit service_sample(uint32_t _cycle)
  ...
  bool init() {
    ...
    app_->register_message_handler(
        ...
        [this](auto &&PH1) { on_get(std::forward<decltype(PH1)>(PH1)); });

    app_->register_message_handler(
        SAMPLE_SERVICE_ID, SAMPLE_INSTANCE_ID, SAMPLE_SET_METHOD_ID,
        [this](auto &&PH1) { on_set(std::forward<decltype(PH1)>(PH1)); });

    std::set<vsomeip::eventgroup_t> its_groups;
    its_groups.insert(SAMPLE_EVENTGROUP_ID);
    app_->offer_event(SAMPLE_SERVICE_ID, SAMPLE_INSTANCE_ID, SAMPLE_EVENT_ID,
    ...
    {
      std::lock_guard<std::mutex> _its_lock(payload_mutex_);
  ...
  void on_set(const std::shared_ptr<vsomeip::message> &_message) {
    ...
    app_->send(its_response);
    app_->notify(SAMPLE_SERVICE_ID, SAMPLE_INSTANCE_ID, SAMPLE_EVENT_ID,
                 payload_);
  }
