 ...
 public:
  ...
  bool init() {
    ...
    std::cout << "Client settings [protocol=" << (use_tcp_ ? "TCP" : "UDP")
              << ":quiet=" << (be_quiet_ ? "true" : "false")
              << ":cycle=" << cycle_ << "]" << std::endl;
    ...
    app_->register_availability_handler(
        SAMPLE_SERVICE_ID + 1, SAMPLE_INSTANCE_ID,
        std::bind(&client_sample::on_availability, this, std::placeholders::_1,
                  std::placeholders::_2, std::placeholders::_3));
    return true;
  }
  ...
  void on_message(const std::shared_ptr<vsomeip::message> &_response) {
    std::cout << "Received a response from Service [" << std::setfill('0')
              << std::hex << std::setw(4) << _response->get_service() << "."
              << std::setw(4) << _response->get_instance()
              << "] to Client/Session [" << std::setw(4)
              << _response->get_client() << "/" << std::setw(4)
              << _response->get_session() << "]" << std::endl;
    if (is_available_) send();
  }
    ...
    while (running_) {
      {
        ...
        if (is_available_) {
          app_->send(request_);
          std::cout << "Client/Session [" << std::setfill('0') << std::hex
