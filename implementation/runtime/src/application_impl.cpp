...
application_impl::~application_impl() {
  ...
  try {
    ...
    std::cerr << __func__ << " catched exception (shutdown): " << e.what()
              << std::endl;
  }
  ...
  try {
    ...
    std::cerr << __func__ << " catched exception (io threads): " << e.what()
              << std::endl;
  }
  ...
  try {
    ...
    std::cerr << __func__ << " catched exception (dispatchers): " << e.what()
              << std::endl;
  }
}
...
bool application_impl::init() {
  ...
  if (is_initialized_) {
    VSOMEIP_WARNING
        << "Trying to initialize an already initialized application.";
    return true;
  }
  ...
  if (nullptr != its_config_module) {
    ...
    if (its_plugin) {
      ...
      if (its_configuration_plugin) {
        configuration_ =
            its_configuration_plugin->get_configuration(name_, path_);
...
#if VSOMEIP_BOOST_VERSION < 106600
      sec_client_.host = htonl(
          static_cast<std::uint32_t>(its_guest_address.to_v4().to_ulong()));
#else
    ...
    if (configuration_->is_security_external()) {
      if (security::load()) {
        ...
        if (VSOMEIP_SEC_POLICY_OK != its_result)
          VSOMEIP_ERROR
              << "Intializing external security implementation failed ("
              << std::dec << its_result << ')';
      }
    } else {
  ...
  if (client_side_logging != nullptr) {
    ...
    if ('\0' != *client_side_logging) {
      ...
      do {
        ...
        const std::stringstream::int_type c =
            its_converter.eof() ? '\0' : its_converter.get();
        switch (c) {
          case '"':
          case '.':
          case ':':
          ...
          case '\0': {
            if ('.' != c) {
              if (0xffffu == prev_val) {
                VSOMEIP_INFO << "+filter " << std::hex << std::setfill('0')
                             << std::setw(4) << val;
                client_side_logging_filter_.insert(
                    std::make_tuple(val, ANY_INSTANCE));
              } else {
                VSOMEIP_INFO << "+filter " << std::hex << std::setfill('0')
                             << std::setw(4) << prev_val << "." << std::setw(4)
                             << val;
                client_side_logging_filter_.insert(
                    std::make_tuple(prev_val, val));
              }
              val = 0xffffu;
            }
          } break;
  ...
  if (its_configuration) {
    VSOMEIP_INFO << "Initializing vsomeip (" VSOMEIP_VERSION ") application \""
                 << name_ << "\".";
    ...
    if (is_routing_manager_host_) {
      ...
      routing_ = std::make_shared<routing_manager_client>(
          this, client_side_logging_, client_side_logging_filter_);
    }
    ...
    VSOMEIP_INFO << "Application(" << (name_ != "" ? name_ : "unnamed") << ", "
                 ...
                 << ") is initialized (" << std::dec << max_dispatchers_ << ", "
                 << max_dispatch_time_ << ").";
  ...
  if (is_initialized_) {
    ...
    auto its_signal_handler = [this](boost::system::error_code const &_error,
                                     int _signal) {
      if (!_error) {
        switch (_signal) {
          case SIGTERM:
  ...
  if (configuration_) {
    ...
    auto its_app_plugin_info =
        its_plugins.find(plugin_type_e::APPLICATION_PLUGIN);
      ...
      for (auto its_library : its_app_plugin_info->second) {
        ...
        if (its_application_plugin) {
          VSOMEIP_INFO << "Client 0x" << std::hex << get_client()
                       << " Loading plug-in library: " << its_library
                       << " succeeded!";
          std::dynamic_pointer_cast<application_plugin>(its_application_plugin)
              ->on_application_state_change(
                  name_, application_plugin_state_e::STATE_INITIALIZED);
        }
      }
    }
  } else {
...
void application_impl::start() {
  ...
  if (getpid() != static_cast<pid_t>(syscall(SYS_gettid))) {
    ...
    std::stringstream s;
    s << std::hex << std::setfill('0') << std::setw(4) << client_ << "_io"
      << std::setw(2) << 0;
  ...
  const int io_thread_nice_level =
      configuration_->get_io_thread_nice_level(name_);
  {
    ...
    for (size_t i = 0; i < io_thread_count - 1; i++) {
      std::shared_ptr<std::thread> its_thread =
          std::make_shared<std::thread>([this, i, io_thread_nice_level] {
            VSOMEIP_INFO << "io thread id from application: " << std::hex
                         << std::setw(4) << std::setfill('0') << client_ << " ("
                         << name_ << ") is: " << std::hex
            ...
            if ((VSOMEIP_IO_THREAD_NICE_LEVEL != io_thread_nice_level) &&
              ...
              VSOMEIP_WARNING << "nice(" << io_thread_nice_level << ") failed "
                              << errno << " for " << std::this_thread::get_id();
            }
#endif
            while (true) {
              try {
  ...
  if (its_app_plugin_info != its_plugins.end()) {
    for (const auto &its_library : its_app_plugin_info->second) {
      ...
      if (its_application_plugin) {
        std::dynamic_pointer_cast<application_plugin>(its_application_plugin)
            ->on_application_state_change(
                name_, application_plugin_state_e::STATE_STARTED);
      }
    }
  }
  ...
  while (true) {
    try {
      ...
      VSOMEIP_ERROR << "application_impl::start() caught exception: "
                    << e.what();
    }
  }
  ...
  if (its_app_plugin_info != its_plugins.end()) {
    for (const auto &its_library : its_app_plugin_info->second) {
      ...
      if (its_application_plugin) {
        std::dynamic_pointer_cast<application_plugin>(its_application_plugin)
            ->on_application_state_change(
                name_, application_plugin_state_e::STATE_STOPPED);
      }
    }
  }
...
void application_impl::release_service(service_t _service,
                                       instance_t _instance) {
  if (routing_) routing_->release_service(client_, _service, _instance);
}
...
void application_impl::subscribe(service_t _service, instance_t _instance,
  ...
  if (routing_) {
    ...
    if (check_subscription_state(_service, _instance, _eventgroup, _event)) {
      routing_->subscribe(client_, &sec_client_, _service, _instance,
...
void application_impl::unsubscribe(service_t _service, instance_t _instance,
  ...
  if (routing_)
    routing_->unsubscribe(client_, &sec_client_, _service, _instance,
                          _eventgroup, ANY_EVENT);
}
...
availability_state_e application_impl::is_available_unlocked(
  ...
  auto check_major_minor =
      [&](const available_instance_t::const_iterator &_found_instance) {
...
bool application_impl::are_available(available_t &_available,
                                     service_t _service, instance_t _instance,
                                     major_version_t _major,
                                     minor_version_t _minor) const {
  std::lock_guard<std::mutex> its_lock(availability_mutex_);
  return (are_available_unlocked(_available, _service, _instance, _major,
                                 _minor) == availability_state_e::AS_AVAILABLE);
}

availability_state_e application_impl::are_available_unlocked(
    available_t &_available, service_t _service, instance_t _instance,
  ...
  for (auto its_available_services_it = _available.begin();
    ...
    auto found_available_service =
        available_.find(its_available_services_it->first);
    if (found_available_service != available_.end()) {
      if (_instance == ANY_INSTANCE) {
        // add all available instances
        for (auto its_available_instances_it =
                 found_available_service->second.begin();
             its_available_instances_it !=
             found_available_service->second.end();
             ++its_available_instances_it) {
          _available[its_available_services_it->first]
                    [its_available_instances_it->first];
        }
      } else {
        if (found_available_service->second.find(_instance) !=
            found_available_service->second.end()) {
  ...
  for (auto its_available_services_it = _available.begin();
    ...
    if (found_available_service != available_.end()) {
      ...
      for (auto its_available_instances_it =
        ...
        auto found_available_instance = found_available_service->second.find(
            its_available_instances_it->first);
        if (found_available_instance != found_available_service->second.end()) {
          if (_major == ANY_MAJOR || _major == DEFAULT_MAJOR) {
            ...
            for (auto its_available_major_it =
                     found_available_instance->second.begin();
                 its_available_major_it !=
                 found_available_instance->second.end();
                 ++its_available_major_it) {
              _available[its_available_services_it->first]
                        [its_available_instances_it->first]
                        [its_available_major_it->first];
            }
          } else {
            if (found_available_instance->second.find(_major) !=
                found_available_instance->second.end()) {
              _available[its_available_services_it->first]
                        [its_available_instances_it->first][_major];
            }
          }
        }
      }
      ...
      for (auto its_available_instances_it =
        ...
        if (found_available_instance != found_available_service->second.end()) {
          ...
          for (auto its_available_major_it =
            ...
            auto found_available_major = found_available_instance->second.find(
                its_available_major_it->first);
            if (found_available_major !=
                found_available_instance->second.end()) {
              if ((_minor == ANY_MINOR || _minor == DEFAULT_MINOR ||
                   _minor <= found_available_major->second.first) &&
                  availability_state_e::AS_AVAILABLE ==
                      found_available_major->second.second) {
                // add minor version
                _available[its_available_services_it->first]
                          [its_available_instances_it->first]
                          [its_available_major_it->first] =
...
void application_impl::send(std::shared_ptr<message> _message) {
  ...
  if (client_side_logging_ &&
      (client_side_logging_filter_.empty() ||
       (1 == client_side_logging_filter_.count(
                 std::make_tuple(_message->get_service(), ANY_INSTANCE))) ||
       (1 == client_side_logging_filter_.count(std::make_tuple(
                 _message->get_service(), _message->get_instance()))))) {
    VSOMEIP_INFO << "application_impl::send: (" << std::hex << std::setfill('0')
                 << std::setw(4) << client_ << "): [" << std::setw(4)
                 << _message->get_service() << "." << std::setw(4)
                 << _message->get_instance() << "." << std::setw(4)
                 << _message->get_method() << ":" << std::setw(4)
                 << (is_request ? session_ : _message->get_session()) << ":"
                 << std::setw(4)
                 << (is_request ? client_.load() : _message->get_client())
                 << "] "
                 << "type="
                 << static_cast<std::uint32_t>(_message->get_message_type())
...
void application_impl::notify(service_t _service, instance_t _instance,
                              ...
                              bool _force) const {
  if (routing_) routing_->notify(_service, _instance, _event, _payload, _force);
}
...
void application_impl::register_availability_handler(
    service_t _service, instance_t _instance,
    const availability_handler_t &_handler, major_version_t _major,
    minor_version_t _minor) {
  ...
  register_availability_handler_unlocked(_service, _instance, its_handler_ext,
                                         _major, _minor);
}
...
void application_impl::register_availability_handler(
    ...
    minor_version_t _minor) {
  std::lock_guard<std::mutex> availability_lock(availability_mutex_);
  register_availability_handler_unlocked(_service, _instance, _handler, _major,
                                         _minor);
}

void application_impl::register_availability_handler_unlocked(
    service_t _service, instance_t _instance,
    availability_state_handler_t _handler, major_version_t _major,
    minor_version_t _minor) {
  if (state_ == state_type_e::ST_REGISTERED) {
    available_t its_available;
    auto are_available = are_available_unlocked(its_available, _service,
    ...
    std::shared_ptr<sync_handler> its_sync_handler =
        std::make_shared<sync_handler>(
            [_handler, are_available, its_available]() {
              for (const auto &available_services_it : its_available)
                for (const auto &available_instances_it :
                     available_services_it.second)
                  _handler(available_services_it.first,
                           available_instances_it.first, are_available);
            });
...
void application_impl::on_subscription(
  ...
  std::pair<subscription_handler_sec_t, async_subscription_handler_sec_t>
      its_handlers;
  {
...
void application_impl::register_subscription_handler(
    service_t _service, instance_t _instance, eventgroup_t _eventgroup,
...
void application_impl::register_subscription_handler(
  ...
  subscription_handler_sec_t its_handler_sec =
      [_handler](client_t _client, const vsomeip_sec_client_t *_sec_client,
                 const std::string &_env, bool _is_subscribed) {
...
void application_impl::register_subscription_handler(
  ...
  std::lock_guard<std::mutex> its_lock(subscription_mutex_);
  subscription_[_service][_instance][_eventgroup] =
      std::make_pair(_handler, nullptr);
}

void application_impl::unregister_subscription_handler(
    service_t _service, instance_t _instance, eventgroup_t _eventgroup) {
  std::lock_guard<std::mutex> its_lock(subscription_mutex_);
  auto found_service = subscription_.find(_service);
  if (found_service != subscription_.end()) {
...
void application_impl::on_subscription_status(service_t _service,
      ...
      if (its_instance != its_service->second.end()) {
        ...
        if (its_eventgroup != its_instance->second.end()) {
          ...
          if (its_event != its_eventgroup->second.end()) {
            entry_found = true;
            its_event->second =
                (_error ? subscription_state_e::SUBSCRIPTION_NOT_ACKNOWLEDGED
  ...
  if (entry_found) {
    deliver_subscription_state(_service, _instance, _eventgroup, _event,
                               _error);
  }
}

void application_impl::deliver_subscription_state(service_t _service,
                                                  instance_t _instance,
                                                  eventgroup_t _eventgroup,
                                                  event_t _event,
                                                  uint16_t _error) {
...
void application_impl::register_subscription_status_handler(
    service_t _service, instance_t _instance, eventgroup_t _eventgroup,
    event_t _event, subscription_status_handler_t _handler,
    bool _is_selective) {
  std::lock_guard<std::mutex> its_lock(subscription_status_handlers_mutex_);
  if (_handler) {
...
void application_impl::unregister_subscription_status_handler(
    service_t _service, instance_t _instance, eventgroup_t _eventgroup,
    event_t _event) {
  std::lock_guard<std::mutex> its_lock(subscription_status_handlers_mutex_);
...
void application_impl::offer_event(
    ...
    bool _update_on_change, const epsilon_change_func_t &_epsilon_change_func,
    reliability_type_e _reliability) {
  if (routing_) {
    if (_cycle == std::chrono::milliseconds::zero() &&
        _change_resets_cycle == false && _update_on_change == true) {
      configuration_->get_event_update_properties(
          _service, _instance, _notifier, _cycle, _change_resets_cycle,
          _update_on_change);

      VSOMEIP_INFO << __func__ << std::hex << std::setfill('0') << ": Event ["
                   << std::setw(4) << _service << "." << std::setw(4)
                   << _instance << "." << std::setw(4) << _notifier
                   << "] uses configured cycle time " << std::dec
                   << _cycle.count() << "ms";
    }

    routing_->register_event(client_, _service, _instance, _notifier,
                             _eventgroups, _type, _reliability, _cycle,
                             _change_resets_cycle, _update_on_change,
                             _epsilon_change_func, true);
  }
}

void application_impl::stop_offer_event(service_t _service,
...
void application_impl::request_event(service_t _service, instance_t _instance,
  ...
  if (routing_)
    routing_->register_event(
        client_, _service, _instance, _event, _eventgroups, _type, _reliability,
        std::chrono::milliseconds::zero(), false, true, nullptr, false);
}
...
const std::string &application_impl::get_name() const { return name_; }

client_t application_impl::get_client() const { return client_; }
...
session_t application_impl::get_session(bool _is_request) {
  if (!has_session_handling_ && !_is_request) return 0;
...
boost::asio::io_context &application_impl::get_io() { return io_; }
              ...
              for (const auto &its_minor : its_major.second) {
                if (!its_minor.second.second) {
                  register_availability_handler_unlocked(
                      ...
                      its_minor.second.first, its_major.first, its_minor.first);
                }
              }
            }
          }
    ...
    auto find_matching_handler =
        [&](const availability_major_minor_t &_av_ma_mi_it) {
          ...
          if (found_major != _av_ma_mi_it.end()) {
            for (std::int32_t mi = static_cast<std::int32_t>(_minor); mi >= 0;
                 mi--) {
              const auto found_minor =
                  found_major->second.find(static_cast<minor_version_t>(mi));
              if (found_minor != found_major->second.end()) {
...
void application_impl::find_service_handlers(
    std::deque<message_handler_t> &_handlers, service_t _service,
    instance_t _instance, method_t _method) const {
  auto its_service_it = members_.find(_service);
  if (its_service_it != members_.end()) {
    find_instance_handlers(_handlers, its_service_it, _instance, _method);
    if (_handlers.empty()) {
      find_instance_handlers(_handlers, its_service_it, ANY_INSTANCE, _method);
    }
  }
}

void application_impl::find_instance_handlers(
    std::deque<message_handler_t> &_handlers, const members_iterator_t &_it,
    instance_t _instance, method_t _method) const {
...
void application_impl::on_message(std::shared_ptr<message> &&_message) {
  ...
  if (_message->get_message_type() == message_type_e::MT_NOTIFICATION) {
    if (!check_for_active_subscription(its_service, its_instance,
      ...
      VSOMEIP_INFO << "application_impl::on_message [" << std::hex
                   << std::setfill('0') << std::setw(4) << its_service << "."
                   << std::setw(4) << its_instance << "." << std::setw(4)
                   << its_method << "]"
...

  {
    ...
    if (its_handlers.empty())
      find_service_handlers(its_handlers, ANY_SERVICE, its_instance,
                            its_method);
    ...
    if (its_handlers.size()) {
      ...
      for (const auto &handler : its_handlers) {
        std::shared_ptr<sync_handler> its_sync_handler =
            std::make_shared<sync_handler>(
                [handler, _message]() { handler(_message); });
...
void application_impl::main_dispatch() {
  ...
  VSOMEIP_INFO << "main dispatch thread id from application: " << std::hex
               << std::setw(4) << std::setfill('0') << client_ << " (" << name_
               << ") is: " << std::hex << its_id
#if defined(__linux__) || defined(ANDROID)
               << " TID: " << std::dec << static_cast<int>(syscall(SYS_gettid))
#endif
...
#endif
  const std::thread::id its_id = std::this_thread::get_id();
  VSOMEIP_INFO << "dispatch thread id from application: " << std::hex
               << std::setw(4) << std::setfill('0') << client_ << " (" << name_
               << ") is: " << std::hex << its_id
#if defined(__linux__) || defined(ANDROID)
               << " TID: " << std::dec << static_cast<int>(syscall(SYS_gettid))
...
std::shared_ptr<application_impl::sync_handler>
application_impl::get_next_handler() {
  ...
  while (!handlers_.empty() && !its_next_handler) {
    ...
    if (its_next_handler->handler_type_ == handler_type_e::AVAILABILITY) {
      const std::pair<service_t, instance_t> its_si_pair = std::make_pair(
          its_next_handler->service_id_, its_next_handler->instance_id_);
...
void application_impl::reschedule_availability_handler(
  ...
  if (_handler->handler_type_ == handler_type_e::AVAILABILITY) {
    const std::pair<service_t, instance_t> its_si_pair =
        std::make_pair(_handler->service_id_, _handler->instance_id_);
    auto found_si = availability_handlers_.find(its_si_pair);
    if (found_si != availability_handlers_.end()) {
      if (!found_si->second.empty() && found_si->second.front() == _handler) {
        ...
        for (auto it = found_si->second.rbegin(); it != found_si->second.rend();
             it++) {
...
void application_impl::invoke_handler(std::shared_ptr<sync_handler> &_handler) {
  ...
  std::shared_ptr<sync_handler> its_sync_handler =
      std::make_shared<sync_handler>(
          _handler->service_id_, _handler->instance_id_, _handler->method_id_,
          _handler->session_id_, _handler->eventgroup_id_,
          _handler->handler_type_);

  boost::asio::steady_timer its_dispatcher_timer(io_);
  its_dispatcher_timer.expires_from_now(
      std::chrono::milliseconds(max_dispatch_time_));
  its_dispatcher_timer.async_wait([this, its_sync_handler](
        ...
        while (is_dispatching_) {
          if (dispatcher_mutex_.try_lock()) {
            if (dispatchers_.size() < max_dispatchers_) {
              ...
              VSOMEIP_ERROR
                  << "Maximum number of dispatchers exceeded. Configuration: "
  ...
  if (client_side_logging_ &&
      (client_side_logging_filter_.empty() ||
       (1 == client_side_logging_filter_.count(std::make_tuple(
                 its_sync_handler->service_id_, ANY_INSTANCE))) ||
       (1 ==
        client_side_logging_filter_.count(std::make_tuple(
            its_sync_handler->service_id_, its_sync_handler->instance_id_))))) {
    VSOMEIP_INFO << "Invoking handler: (" << std::hex << std::setfill('0')
                 << std::setw(4) << client_ << "): [" << std::setw(4)
                 << its_sync_handler->service_id_ << "." << std::setw(4)
                 << its_sync_handler->instance_id_ << "." << std::setw(4)
                 << its_sync_handler->method_id_ << ":" << std::setw(4)
                 << its_sync_handler->session_id_ << "] "
                 << "type="
                 << static_cast<std::uint32_t>(its_sync_handler->handler_type_)
...
void application_impl::remove_elapsed_dispatchers() {
  if (is_dispatching_) {
    ...
    for (auto id : elapsed_dispatchers_) {
      ...
      if (its_dispatcher->second->joinable()) its_dispatcher->second->join();
...
void application_impl::shutdown() {
  VSOMEIP_INFO << "shutdown thread id from application: " << std::hex
               << std::setw(4) << std::setfill('0') << client_ << " (" << name_
...
bool application_impl::is_routing() const { return is_routing_manager_host_; }

void application_impl::send_back_cached_event(service_t _service,
                                              instance_t _instance,
                                              event_t _event) {
  std::shared_ptr<event> its_event =
      routing_->find_event(_service, _instance, _event);
  if (its_event && its_event->is_field() && its_event->is_set()) {
    ...
    VSOMEIP_INFO << "Sending back cached event (" << std::hex
...
void application_impl::send_back_cached_eventgroup(service_t _service,
  ...
  std::set<std::shared_ptr<event>> its_events =
      routing_->find_events(_service, _instance, _eventgroup);
...
void application_impl::set_routing_state(routing_state_e _routing_state) {
  if (routing_) routing_->set_routing_state(_routing_state);
}
...
void application_impl::remove_subscription(service_t _service,
                                           ...
                                           event_t _event) {
  {
    ...
    if (its_service != subscription_state_.end()) {
      ...
      if (its_service->second.empty()) subscription_state_.erase(its_service);
    }
  }
...
bool application_impl::check_subscription_state(service_t _service,
                                                instance_t _instance,
                                                eventgroup_t _eventgroup,
                                                event_t _event) {
...
void application_impl::print_blocking_call(
    const std::shared_ptr<sync_handler> &_handler) {
  switch (_handler->handler_type_) {
    case handler_type_e::AVAILABILITY:
      VSOMEIP_WARNING << "BLOCKING CALL AVAILABILITY(" << std::hex
    ...
    case handler_type_e::MESSAGE:
      VSOMEIP_WARNING << "BLOCKING CALL MESSAGE(" << std::hex
                      << std::setfill('0') << std::setw(4) << get_client()
                      << "): [" << std::setw(4) << _handler->service_id_ << "."
                      << std::setw(4) << _handler->instance_id_ << "."
    ...
    case handler_type_e::STATE:
      VSOMEIP_WARNING << "BLOCKING CALL STATE(" << std::hex << std::setw(4)
                      << std::setfill('0') << get_client() << ")";
      break;
    case handler_type_e::SUBSCRIPTION:
      VSOMEIP_WARNING << "BLOCKING CALL SUBSCRIPTION(" << std::hex
                      << std::setfill('0') << std::setw(4) << get_client()
                      << "): [" << std::setw(4) << _handler->service_id_ << "."
                      << std::setw(4) << _handler->instance_id_ << "."
    ...
    case handler_type_e::OFFERED_SERVICES_INFO:
      VSOMEIP_WARNING << "BLOCKING CALL OFFERED_SERVICES_INFO(" << std::hex
                      << std::setw(4) << std::setfill('0') << get_client()
                      << ")";
      break;
    case handler_type_e::WATCHDOG:
      VSOMEIP_WARNING << "BLOCKING CALL WATCHDOG(" << std::hex << std::setw(4)
                      << std::setfill('0') << get_client() << ")";
      break;
    case handler_type_e::UNKNOWN:
      VSOMEIP_WARNING << "BLOCKING CALL UNKNOWN(" << std::hex << std::setw(4)
                      << std::setfill('0') << get_client() << ")";
      break;
  }
}

void application_impl::get_offered_services_async(
    offer_type_e _offer_type, const offered_services_handler_t &_handler) {
  {
    std::lock_guard<std::mutex> its_lock(offered_services_handler_mutex_);
  ...
  if (!is_routing_manager_host_) {
    ...
    auto its_routing_manager_host =
        std::dynamic_pointer_cast<routing_manager_impl>(routing_);
      ...
      for (const auto &i : s.second) {
        ...
        if (_offer_type == offer_type_e::OT_LOCAL) {
          if (((its_unreliable_endpoint &&
                (its_unreliable_endpoint->get_local_port() == ILLEGAL_PORT)) &&
               (its_reliable_endpoint &&
                (its_reliable_endpoint->get_local_port() == ILLEGAL_PORT))) ||
...
void application_impl::on_offered_services_info(
    std::vector<std::pair<service_t, instance_t>> &_services) {
  ...
  if (has_offered_services_handler) {
    ...
    std::shared_ptr<sync_handler> its_sync_handler =
        std::make_shared<sync_handler>(
            [handler, _services]() { handler(_services); });
...
void application_impl::set_watchdog_handler(const watchdog_handler_t &_handler,
  ...
  if (_handler && std::chrono::seconds::zero() != _interval) {
    ...
    watchdog_timer_.async_wait(std::bind(&application_impl::watchdog_cbk, this,
                                         std::placeholders::_1));
  } else {
    std::lock_guard<std::mutex> its_lock(watchdog_timer_mutex_);
...
void application_impl::register_async_subscription_handler(
    service_t _service, instance_t _instance, eventgroup_t _eventgroup,
...
void application_impl::register_async_subscription_handler(
  ...
  async_subscription_handler_sec_t its_handler_sec =
      [_handler](client_t _client, const vsomeip_sec_client_t *_sec_client,
                 const std::string &_env, bool _is_subscribed,
...
void application_impl::register_async_subscription_handler(
  ...
  std::lock_guard<std::mutex> its_lock(subscription_mutex_);
  subscription_[_service][_instance][_eventgroup] =
      std::make_pair(nullptr, _handler);
}
...
void application_impl::set_sd_acceptance_required(const remote_info_t &_remote,
  ...
  const boost::asio::ip::address its_address(
      _remote.ip_.is_v4_
          ? static_cast<boost::asio::ip::address>(
                boost::asio::ip::address_v4(_remote.ip_.address_.v4_))
          : static_cast<boost::asio::ip::address>(
                boost::asio::ip::address_v6(_remote.ip_.address_.v6_)));
  ...
  configuration_->set_sd_acceptance_rule(its_address, its_range,
                                         port_type_e::PT_UNKNOWN, _path,
                                         _remote.is_reliable_, _enable, true);
...
bool application_impl::update_service_configuration(
    service_t _service, instance_t _instance, std::uint16_t _port,
    bool _reliable, bool _magic_cookies_enabled, bool _offer) {
  bool ret = false;
  if (!is_routing_manager_host_) {
    ...
    if (rm_impl) {
      if (_offer) {
        ret = rm_impl->offer_service_remotely(
            _service, _instance, _port, _reliable, _magic_cookies_enabled);
      } else {
        ret = rm_impl->stop_offer_service_remotely(
            _service, _instance, _port, _reliable, _magic_cookies_enabled);
      }
    }
  }
...
void application_impl::update_security_policy_configuration(
  ...
  if (!is_routing()) {
    ...
    auto rm_impl = std::dynamic_pointer_cast<routing_manager_impl>(routing_);
    if (rm_impl) {
      rm_impl->update_security_policy_configuration(_uid, _gid, _policy,
                                                    _payload, _handler);
    }
  }
...
void application_impl::remove_security_policy_configuration(
...
void application_impl::subscribe_with_debounce(
    service_t _service, instance_t _instance, eventgroup_t _eventgroup,
    major_version_t _major, event_t _event, const debounce_filter_t &_filter) {
  if (routing_) {
    bool send_back_cached(false);
    bool send_back_cached_group(false);
    check_send_back_cached_event(_service, _instance, _event, _eventgroup,
    ...
    if (check_subscription_state(_service, _instance, _eventgroup, _event)) {
      ...
      routing_->subscribe(client_, get_sec_client(), _service, _instance,
                          _eventgroup, _major, _event, its_filter);
    }
  }
}

bool application_impl::is_local_endpoint(
    const boost::asio::ip::address &_unicast, port_t _port) {
  try {
...
std::map<std::string, std::string> application_impl::get_additional_data(
