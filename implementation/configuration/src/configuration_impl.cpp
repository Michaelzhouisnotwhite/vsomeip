...
configuration_impl::configuration_impl(const std::string &_path)
    : default_unicast_("local"),
      ...
      npdu_default_max_retention_resp_(
          VSOMEIP_DEFAULT_NPDU_MAXIMUM_RETENTION_NANO),
  ...
  for (auto i = 0; i < ET_MAX; i++) is_configured_[i] = false;
...
configuration_impl::configuration_impl(const configuration_impl &_other)
    : std::enable_shared_from_this<configuration_impl>(_other),
      ...
      path_(_other.path_) {
  applications_.insert(_other.applications_.begin(),
                       _other.applications_.end());
  ...
  magic_cookies_.insert(_other.magic_cookies_.begin(),
                        _other.magic_cookies_.end());
...
bool configuration_impl::load(const std::string &_name) {
  ...
  std::string its_file(
      VSOMEIP_DEFAULT_CONFIGURATION_FILE);  // configuration file
  std::string its_folder(
      VSOMEIP_DEFAULT_CONFIGURATION_FOLDER);  // configuration folder
  ...
  if (nullptr == its_env) its_env = getenv(VSOMEIP_ENV_CONFIGURATION);
  if (nullptr != its_env) {
  ...
  if (its_folder != "") {
    ...
    its_security_config_folder << its_folder << "/" << getuid() << "_"
                               << getgid();
  ...
  // If the configuration is incomplete, this is the routing manager
  // configuration or the routing is yet unknown, read the full set of
  // configuration files
  ...
  for (const auto &f : its_failed)
    VSOMEIP_WARNING << "Reading of configuration file \"" << f
                    << "\" failed. Configuration may be incomplete.";
...
bool configuration_impl::lazy_load_security(const std::string &_client_host) {
  ...
  std::string its_folder =
      policy_manager_impl::get()->get_policy_extension_path(_client_host);
  if (!its_folder.empty()) {
    std::set<std::string> its_input;
    std::set<std::string> its_failed;
    std::vector<configuration_element> its_mandatory_elements;
    ...
    its_input.insert(its_folder);
    // load security configuration files from UID_GID sub folder if existing
    std::string its_security_config_folder =
        policy_manager_impl::get()->get_security_config_folder(its_folder);
    ...
    for (auto f : its_failed)
      VSOMEIP_WARNING << __func__ << ": Reading of configuration file \"" << f
                      << "\" failed. Configuration may be incomplete.";

    result = (its_failed.empty() && !its_mandatory_elements.empty());
    if (result)
      policy_manager_impl::get()->set_is_policy_extension_loaded(_client_host,
                                                                 true);
  }
...
bool configuration_impl::check_routing_credentials(
...
bool configuration_impl::remote_offer_info_add(service_t _service,
  ...
  if (!is_loaded_) {
    ...
    _reliable ? its_service->reliable_ = _port
...

    {
      ...
      if (found_service != services_.end()) {
        auto found_instance =
            found_service->second.find(its_service->instance_);
      ...
      if (!updated) {
        ...
        VSOMEIP_INFO << "Added new remote configuration for service ["
                     ...
                     << its_service->service_ << "." << std::setw(4)
...
bool configuration_impl::remote_offer_info_remove(
    service_t _service, instance_t _instance, std::uint16_t _port,
    bool _reliable, bool _magic_cookies_enabled, bool *_still_offered_remote) {
  (void)_port;
    ...
    if (utility::is_file(i)) {
      ...
      for (auto j = boost::filesystem::directory_iterator(its_path);
        ...
        if (!boost::filesystem::is_directory(j->path())) {
          ...
          if (utility::is_file(name)) its_names[name] = true;
        }
      }
...
void configuration_impl::load_policy_data(
    const std::string &_input, std::vector<configuration_element> &_elements,
    std::set<std::string> &_failed, bool _mandatory_only) {
...
bool configuration_impl::load_data(
    const std::vector<configuration_element> &_elements, bool _load_mandatory,
    bool _load_optional) {
  ...
  if (!is_logging_loaded_) {
    ...
    if (is_logging_loaded_) {
      logger::logger_impl::init(shared_from_this());
      for (const auto &w : its_warnings) VSOMEIP_WARNING << w;
    }
  }
...
bool configuration_impl::load_logging(const configuration_element &_element,
    ...
    for (auto i = its_logging.begin(); i != its_logging.end(); ++i) {
      ...
      if (its_key == "console") {
        if (is_configured_[ET_LOGGING_CONSOLE]) {
          _warnings.insert(
              "Multiple definitions for logging.console."
        ...
        if (is_configured_[ET_LOGGING_FILE]) {
          _warnings.insert(
              "Multiple definitions for logging.file."
              " Ignoring definition from " +
              _element.name_);
        } else {
          for (auto j : i->second) {
        ...
        if (is_configured_[ET_LOGGING_DLT]) {
          _warnings.insert(
              "Multiple definitions for logging.dlt."
              " Ignoring definition from " +
              _element.name_);
        } else {
          std::string its_value(i->second.data());
        ...
        if (is_configured_[ET_LOGGING_LEVEL]) {
          _warnings.insert(
              "Multiple definitions for logging.level."
              " Ignoring definition from " +
              _element.name_);
        } else {
          std::string its_value(i->second.data());
  ...
  try {
    ...
    if (is_configured_[ET_ROUTING]) {
      ...
      routing_.is_enabled_ =
          its_routing.get_optional<bool>("enabled").get_value_or(
              routing_.is_enabled_);
      ...
      for (auto &ug : routing_.guests_.ports_) {
        ...
        for (const auto &r : its_invalid_ranges) ug.second.erase(r);
        its_invalid_ranges.clear();

        for (const auto &r : its_corrected_ranges) ug.second.insert(r);
...
bool configuration_impl::load_routing_host(
    const boost::property_tree::ptree &_tree, const std::string &_name) {
  try {
    ...
    if (has_uid && has_gid) {
      policy_manager_impl::get()->set_routing_credentials(its_uid, its_gid,
                                                          _name);
    }
...
bool configuration_impl::load_routing_guests(
...
void configuration_impl::load_routing_guest_ports(
    const boost::property_tree::ptree &_tree) {
  std::stringstream its_converter;
  ...
  for (auto its_range = _tree.begin(); its_range != _tree.end(); ++its_range) {
    ...
    try {
      ...
      its_converter >> (its_uid_value.find("0x") == 0 ? std::hex : std::dec) >>
          its_uid;
      ...
      its_converter >> (its_gid_value.find("0x") == 0 ? std::hex : std::dec) >>
          its_gid;
  ...
  for (auto its_range = _tree.begin(); its_range != _tree.end(); ++its_range) {
    ...
    for (auto its_element = its_range->second.begin();
      ...
      if (!its_value.empty()) {
        its_converter.str("");
        its_converter.clear();
        its_converter << (its_value.find("0x") == 0 ? std::hex : std::dec)
                      << its_value;
...
void configuration_impl::load_routing_client_ports(
...
bool configuration_impl::load_applications(
    const configuration_element &_element) {
  try {
...
void configuration_impl::load_application_data(
  ...
  if (its_name != "") {
    if (applications_.find(its_name) == applications_.end()) {
      ...
      VSOMEIP_WARNING << "Multiple configurations for application " << its_name
    ...
    for (auto l = i->second.begin(); l != i->second.end(); ++l) {
      ...
      if (its_inner_key == "name") {
        ...
        if (its_configured[ET_PLUGIN_NAME]) {
          ...
          for (auto m = its_additional_tree.begin();
               m != its_additional_tree.end(); ++m) {
...
void configuration_impl::add_plugin(
    std::map<plugin_type_e, std::set<std::string>> &_plugins,
  ...
  if (_plugin_data.type_ == "application_plugin") {
    ...
    VSOMEIP_WARNING << "Unknown plug-in type (" << _plugin_data.type_
                    << ") configured for client: " << _application_name;
  }
}
...
void configuration_impl::load_tracing(const configuration_element &_element) {
  try {
    ...
    for (auto i = its_trace_configuration.begin();
         i != its_trace_configuration.end(); ++i) {
      std::string its_key(i->first);
      std::string its_value(i->second.data());
      if (its_key == "enable") {
...
void configuration_impl::load_trace_channels(
  ...
  try {
    for (auto i = _tree.begin(); i != _tree.end(); ++i) {
      if (i == _tree.begin()) trace_->channels_.clear();
...
void configuration_impl::load_trace_channel(
    const boost::property_tree::ptree &_tree) {
  std::shared_ptr<trace_channel> its_channel =
      std::make_shared<trace_channel>();
  for (auto i = _tree.begin(); i != _tree.end(); ++i) {
    std::string its_key = i->first;
    std::string its_value = i->second.data();
    if (its_key == "name") {
...
void configuration_impl::load_trace_filter_expressions(
  ...
  if (_criteria == "services") {
    ...
    if (!has_issued_methods_warning_) {
      VSOMEIP_WARNING
          << "\"method\" entry in filter configuration has no effect!";
      has_issued_methods_warning_ = true;
    }
  } else if (_criteria == "clients") {
    if (!has_issued_clients_warning_) {
      VSOMEIP_WARNING
          << "\"clients\" entry in filter configuration has no effect!";
...
void configuration_impl::load_suppress_events(
    const configuration_element &_element) {
  try {
    auto its_missing_events =
        _element.tree_.get_child("suppress_missing_event_logs");

    if (its_missing_events.size()) {
      for (auto i = its_missing_events.begin(); i != its_missing_events.end();
           ++i) {
        load_suppress_events_data(i->second);
      }

      if (suppress_events_.size()) is_suppress_events_enabled_ = true;
    }
  } catch (...) {
    // Intentionally left empty
  }
...
void configuration_impl::load_suppress_events_data(
  ...
  if (events.empty()) events.insert(ANY_EVENT);
      ...
      if (i->second.size() == 0) {
        if (its_key == "first") {  // Range Events
          ...
          std::set<event_t> range_events =
              load_range_events(its_first_event, its_last_event);
        ...
        std::set<event_t> range_events =
            load_suppress_multiple_events(i->second);
        events.insert(range_events.begin(), range_events.end());
      }
    }
  } catch (...) {
...
uint16_t configuration_impl::load_suppress_data(
...
std::set<event_t> configuration_impl::load_range_events(
    event_t _first_event, event_t _last_event) const {
  std::set<event_t> range_events;

  if ((_first_event != ANY_EVENT) && (_last_event != ANY_EVENT)) {
    for (event_t its_event = _first_event; its_event <= _last_event;
         its_event++) {
...
bool configuration_impl::check_suppress_events(service_t _service,
                                               instance_t _instance,
                                               event_t _event) const {
  if (!is_suppress_events_enabled_) return false;
...
void configuration_impl::print_suppress_events(void) const {
  ...
  for (const auto &its_log : suppress_events_) {
    VSOMEIP_INFO << std::hex << std::setfill('0') << "+[" << std::setw(4)
                 << its_log.service << "." << std::setw(4) << its_log.instance
                 << "." << its_log.event << "]";
  }
}

void configuration_impl::load_unicast_address(
    const configuration_element &_element) {
  try {
...
void configuration_impl::load_diagnosis_address(
    const configuration_element &_element) {
  try {
  ...
  try {
    if (_element.tree_.get_child_optional(shutdown_timeout)) {
      ...
      if (is_configured_[ET_SHUTDOWN_TIMEOUT]) {
        ...
        if (its_value.size() > 1 && its_value[0] == '0' &&
            its_value[1] == 'x') {
          its_converter << std::hex << its_value;
        } else {
          its_converter << std::dec << its_value;
    ...
    for (auto i = its_service_discovery.begin();
      ...
      if (its_key == "enable") {
        if (is_configured_[ET_SERVICE_DISCOVERY_ENABLE]) {
          VSOMEIP_WARNING
              << "Multiple definitions for service_discovery.enabled."
                 " Ignoring definition from "
              << _element.name_;
        } else {
        ...
        if (is_configured_[ET_SERVICE_DISCOVERY_MULTICAST]) {
          VSOMEIP_WARNING
              << "Multiple definitions for service_discovery.multicast."
                 " Ignoring definition from "
              << _element.name_;
        } else {
        ...
        if (is_configured_[ET_SERVICE_DISCOVERY_PROTOCOL]) {
          VSOMEIP_WARNING
              << "Multiple definitions for service_discovery.protocol."
                 " Ignoring definition from "
              << _element.name_;
        } else {
        ...
        if (is_configured_[ET_SERVICE_DISCOVERY_INITIAL_DELAY_MIN]) {
          VSOMEIP_WARNING
              << "Multiple definitions for service_discovery.initial_delay_min."
                 " Ignoring definition from "
              << _element.name_;
        } else {
          its_converter << its_value;
        ...
        if (is_configured_[ET_SERVICE_DISCOVERY_INITIAL_DELAY_MAX]) {
          VSOMEIP_WARNING
              << "Multiple definitions for service_discovery.initial_delay_max."
                 " Ignoring definition from "
              << _element.name_;
        } else {
          its_converter << its_value;
        ...
        if (is_configured_[ET_SERVICE_DISCOVERY_REPETITION_BASE_DELAY]) {
          VSOMEIP_WARNING << "Multiple definitions for "
                             "service_discovery.repetition_base_delay."
                             " Ignoring definition from "
                          << _element.name_;
        } else {
          its_converter << its_value;
        ...
        if (is_configured_[ET_SERVICE_DISCOVERY_REPETITION_MAX]) {
          VSOMEIP_WARNING
              << "Multiple definitions for service_discovery.repetition_max."
                 " Ignoring definition from "
              << _element.name_;
        } else {
          int tmp;
        ...
        if (is_configured_[ET_SERVICE_DISCOVERY_CYCLIC_OFFER_DELAY]) {
          VSOMEIP_WARNING << "Multiple definitions for "
                             "service_discovery.cyclic_offer_delay."
                             " Ignoring definition from "
                          << _element.name_;
        } else {
          its_converter << its_value;
        ...
        if (is_configured_[ET_SERVICE_DISCOVERY_REQUEST_RESPONSE_DELAY]) {
          VSOMEIP_WARNING << "Multiple definitions for "
                             "service_discovery.request_response_delay."
                             " Ignoring definition from "
                          << _element.name_;
        } else {
          its_converter << its_value;
        ...
        if (is_configured_[ET_SERVICE_DISCOVERY_OFFER_DEBOUNCE_TIME]) {
          VSOMEIP_WARNING
              << "Multiple definitions for service_discovery.offer_debounce."
                 " Ignoring definition from "
              << _element.name_;
        } else {
          its_converter << its_value;
        ...
        if (is_configured_[ET_SERVICE_DISCOVERY_TTL_FACTOR_OFFERS]) {
          VSOMEIP_WARNING
              << "Multiple definitions for service_discovery.ttl_factor_offers."
                 " Ignoring definition from "
              << _element.name_;
        } else {
        ...
        if (is_configured_[ET_MAX_REMOTE_SUBSCRIBERS]) {
          VSOMEIP_WARNING << "Multiple definitions for "
                             "service_discovery.max_remote_subscribers."
                             " Ignoring definition from "
                          << _element.name_;
        } else {
          int tmp;
          ...
          max_remote_subscribers_ =
              (tmp > std::numeric_limits<std::uint8_t>::max())
                  ? std::numeric_limits<std::uint8_t>::max()
                  : static_cast<std::uint8_t>(tmp);
          if (max_remote_subscribers_ == 0) {
            VSOMEIP_WARNING
                << "max_remote_subscribers_ = 0 is not allowed. Using default ("
  ...
  try {
    ...
    for (auto i = _tree.begin(); i != _tree.end(); ++i) {
      ...
      if (its_key == "initial") {
        ...
        sd_repetitions_max_ =
            (tmp_repetition_max > std::numeric_limits<std::uint8_t>::max())
                ? std::numeric_limits<std::uint8_t>::max()
...
void configuration_impl::load_npdu_default_timings(
    const configuration_element &_element) {
        ...
        for (const auto &e : _element.tree_.get_child(ndt)) {
          ...
          try {
            its_time = std::chrono::nanoseconds(
                std::strtoull(e.second.data().c_str(), NULL, 10) * 1000000);
...
void configuration_impl::load_services(const configuration_element &_element) {
  ...
  try {
    ...
    try {
      ...
      for (auto i = its_servicegroups.begin(); i != its_servicegroups.end();
           ++i)
...
void configuration_impl::load_service(const boost::property_tree::ptree &_tree,
  ...
  try {
    ...
    if (found_service != services_.end()) {
      ...
      if (found_instance != found_service->second.end()) {
        VSOMEIP_WARNING << "Multiple configurations for service [" << std::hex
    ...
    if (is_loaded) {
      ...
      if (its_service->unicast_address_ == default_unicast_) {
        ...
        if (its_service->reliable_ != ILLEGAL_PORT) {
          services_by_ip_port_[unicast_.to_string()][its_service->reliable_]
                              [its_service->service_] = its_service;
        }
        if (its_service->unreliable_ != ILLEGAL_PORT) {
          services_by_ip_port_[unicast_.to_string()][its_service->unreliable_]
                              [its_service->service_] = its_service;
        ...
        if (its_service->reliable_ != ILLEGAL_PORT) {
          services_by_ip_port_[its_service->unicast_address_]
                              [its_service->reliable_][its_service->service_] =
                                  its_service;
        }
        if (its_service->unreliable_ != ILLEGAL_PORT) {
          services_by_ip_port_[its_service->unicast_address_]
...
void configuration_impl::load_event(std::shared_ptr<service> &_service,
                                    const boost::property_tree::ptree &_tree) {
  for (auto i = _tree.begin(); i != _tree.end(); ++i) {
    ...
    if (its_event_id > 0) {
      ...
      if (found_event != _service->events_.end()) {
        VSOMEIP_INFO << "Multiple configurations for event [" << std::hex
                     << _service->service_ << "." << _service->instance_ << "."
                     << its_event_id << "].";
      } else {
        ...
        std::shared_ptr<event> its_event = std::make_shared<event>(
            its_event_id, its_is_field, its_reliability, its_cycle,
            its_change_resets_cycle, its_update_on_change);
        ...
        for (auto k = j->second.begin(); k != j->second.end(); ++k) {
          ...
          if (0 < its_event_id) {
            ...
            if (find_event != _service->events_.end()) {
              ...
              its_event = std::make_shared<event>(
                  its_event_id, false, reliability_type_e::RT_UNRELIABLE,
...
void configuration_impl::load_internal_services(
    const configuration_element &_element) {
  try {
    ...
    for (auto found_range = its_internal_services.begin();
      ...
      for (auto i = found_range->second.begin(); i != found_range->second.end();
           ++i) {
        if (i->first == "first") {
          if (i->second.size() == 0) {
            std::stringstream its_converter;
...
void configuration_impl::load_client(const boost::property_tree::ptree &_tree) {
  try {
    ...
    its_client->remote_ports_[true] =
        std::make_pair(ILLEGAL_PORT, ILLEGAL_PORT);
    its_client->remote_ports_[false] =
        std::make_pair(ILLEGAL_PORT, ILLEGAL_PORT);
    its_client->client_ports_[true] =
        std::make_pair(ILLEGAL_PORT, ILLEGAL_PORT);
    its_client->client_ports_[false] =
        std::make_pair(ILLEGAL_PORT, ILLEGAL_PORT);
...
std::pair<uint16_t, uint16_t> configuration_impl::load_client_port_range(
  ...
  if (its_last_port < its_first_port) {
    VSOMEIP_WARNING << "Port range invalid: first: " << std::dec
                    << its_first_port << " last: " << its_last_port;
    ...
    for (auto i = its_service_discovery.begin();
      ...
      if (its_key == "enable") {
        ...
        if (is_configured_[ET_WATCHDOG_ALLOWED_MISSING_PONGS]) {
          VSOMEIP_WARNING
              << "Multiple definitions of watchdog.allowed_missing_pongs."
                 " Ignoring definition from "
              << _element.name_;
        } else {
          its_converter << std::dec << its_value;
...
void configuration_impl::load_payload_sizes(
    const configuration_element &_element) {
  ...
  try {
    ...
    if (_element.tree_.get_child_optional(buffer_shrink_threshold)) {
      ...
      try {
        buffer_shrink_threshold_ =
            static_cast<std::uint32_t>(std::stoul(s.c_str(), NULL, 10));
      } catch (const std::exception &e) {
    ...
    if (_element.tree_.get_child_optional(payload_sizes)) {
      ...
      for (auto i = its_ps.begin(); i != its_ps.end(); ++i) {
        ...
        for (auto j = i->second.get_child(ports).begin();
          ...
          try {
            ...
            its_port =
                static_cast<std::uint16_t>(std::stoul(p.c_str(), NULL, 10));
      ...
      if (max_local_message_size_ != 0 && max_reliable_message_size_ != 0 &&
...
void configuration_impl::load_permissions(
    const configuration_element &_element) {
...
void configuration_impl::load_security(const configuration_element &_element) {
  try {
    ...
    if (its_security) {
      ...
      auto its_audit_mode =
          its_security->get_child_optional("check_credentials");
      if (its_audit_mode) {
      ...
      auto its_remote_access =
          its_security->get_child_optional("allow_remote_clients");
...
#ifndef VSOMEIP_DISABLE_SECURITY
  if (!is_security_external()) policy_manager_impl::get()->load(_element);
#endif  // !VSOMEIP_DISABLE_SECURITY
}

void configuration_impl::load_selective_broadcasts_support(
    const configuration_element &_element) {
  try {
    auto its_service_discovery =
        _element.tree_.get_child("supports_selective_broadcasts");
    for (auto i = its_service_discovery.begin();
         i != its_service_discovery.end(); ++i) {
      std::string its_value(i->second.data());
...
void configuration_impl::load_partitions(
    const configuration_element &_element) {
  try {
...
void configuration_impl::load_partition(
    const boost::property_tree::ptree &_tree) {
  ...
  try {
    ...
    for (auto i = _tree.begin(); i != _tree.end(); ++i) {
      ...
      if (its_service > 0 && its_instance > 0)
        ...
        VSOMEIP_ERROR << "P: <" << its_service_s << "." << its_instance_s
                      << "> is no valid service instance.";
    }
    ...
    if (!its_partition_members.empty()) {
      ...
      for (const auto &p : its_partition_members) {
        for (const auto &m : p.second) {
          ...
          its_log << "<" << std::setfill('0') << std::hex << std::setw(4)
                  << p.first << "." << std::setw(4) << m << ">";
        }
      }
...
bool configuration_impl::is_in_port_range(
    uint16_t _port, std::pair<uint16_t, uint16_t> _port_range) const {
...
// Public interface
///////////////////////////////////////////////////////////////////////////////
const std::string &configuration_impl::get_network() const { return network_; }

const boost::asio::ip::address &configuration_impl::get_unicast_address()
    const {
  return unicast_;
}
...
unsigned short configuration_impl::get_prefix() const { return prefix_; }

const std::string &configuration_impl::get_device() const { return device_; }
...
bool configuration_impl::is_v4() const { return unicast_.is_v4(); }

bool configuration_impl::is_v6() const { return unicast_.is_v6(); }

bool configuration_impl::has_console_log() const { return has_console_log_; }

bool configuration_impl::has_file_log() const { return has_file_log_; }

bool configuration_impl::has_dlt_log() const { return has_dlt_log_; }

const std::string &configuration_impl::get_logfile() const { return logfile_; }
...
std::string configuration_impl::get_unicast_address(
...
bool configuration_impl::is_someip(service_t _service,
                                   instance_t _instance) const {
  auto its_service = find_service(_service, _instance);
  if (its_service) return (its_service->protocol_ == "someip");
...
bool configuration_impl::get_client_port(
    service_t _service, instance_t _instance, uint16_t _remote_port,
    bool _reliable, std::map<bool, std::set<uint16_t>> &_used_client_ports,
    uint16_t &_client_port) const {
  bool is_configured(false);
  _client_port = ILLEGAL_PORT;

  uint16_t its_specific_port(ILLEGAL_PORT);
  if (find_specific_port(its_specific_port, _service, _instance, _reliable,
                         _used_client_ports)) {
    is_configured = true;
  ...
  VSOMEIP_ERROR << "Cannot find free client port for communication to service ["
                ...
                << "." << std::hex << std::setw(4) << std::setfill('0')
                << _instance << "." << std::dec << _remote_port << "."
...
const boost::asio::ip::address &configuration_impl::get_routing_host_address()
    const {
...
const boost::asio::ip::address &configuration_impl::get_routing_guest_address()
    const {
  return routing_.guests_.unicast_;
}

std::set<std::pair<port_t, port_t>> configuration_impl::get_routing_guest_ports(
    uid_t _uid, gid_t _gid) const {
  auto found = routing_.guests_.ports_.find({_uid, _gid});
  if (found != routing_.guests_.ports_.end()) return found->second;
...
std::size_t configuration_impl::get_request_debouncing(
...
std::size_t configuration_impl::get_io_thread_count(
    const std::string &_name) const {
  std::size_t its_io_thread_count = VSOMEIP_IO_THREAD_COUNT;
...
int configuration_impl::get_io_thread_nice_level(
    const std::string &_name) const {
  int its_io_thread_nice_level = VSOMEIP_IO_THREAD_NICE_LEVEL;
...
bool configuration_impl::is_mandatory(const std::string &_name) const {
  ...
  if (its_candidates.empty()) return false;
...
void configuration_impl::trim(std::string &_s) {
  _s.erase(_s.begin(), std::find_if(_s.begin(), _s.end(), [](const auto ch) {
...
bool configuration_impl::is_remote(
    const std::shared_ptr<service> &_service) const {
...
bool configuration_impl::get_multicast(service_t _service, instance_t _instance,
                                       eventgroup_t _eventgroup,
                                       std::string &_address,
                                       uint16_t &_port) const {
  std::shared_ptr<eventgroup> its_eventgroup =
      find_eventgroup(_service, _instance, _eventgroup);
  if (!its_eventgroup) return false;

  if (its_eventgroup->multicast_address_.empty()) return false;
...
std::shared_ptr<client> configuration_impl::find_client(
    service_t _service, instance_t _instance) const {
...
bool configuration_impl::find_port(
    uint16_t &_port, uint16_t _remote, bool _reliable,
    std::map<bool, std::set<uint16_t>> &_used_client_ports) const {
  bool is_configured(false);
  std::list<std::shared_ptr<client>>::const_iterator it;

  for (it = clients_.begin(); it != clients_.end(); ++it) {
    if (_remote != ILLEGAL_PORT &&
        is_in_port_range(_remote, (*it)->remote_ports_[_reliable])) {
      is_configured = true;
      uint16_t its_port(ILLEGAL_PORT);
...
bool configuration_impl::find_specific_port(
    uint16_t &_port, service_t _service, instance_t _instance, bool _reliable,
    std::map<bool, std::set<uint16_t>> &_used_client_ports) const {
  bool is_configured(false);
  ...
  if (its_client && !its_client->ports_[_reliable].empty()) {
    ...
    if (its_client->last_used_specific_client_port_[_reliable] ==
        ILLEGAL_PORT) {
    ...
    while (it != its_client->ports_[_reliable].end()) {
      if (_used_client_ports[_reliable].find(*it) ==
        ...
        VSOMEIP_INFO << "configuration_impl:find_specific_port #1:"
                     ...
                     << " instance: " << _instance << " reliable: " << std::dec
                     << _reliable
...
reliability_type_e configuration_impl::get_event_reliability(
...
reliability_type_e configuration_impl::get_service_reliability(
    service_t _service, instance_t _instance) const {
  std::lock_guard<std::mutex> its_lock(services_mutex_);
  reliability_type_e its_reliability(reliability_type_e::RT_UNKNOWN);
  auto its_service = find_service_unlocked(_service, _instance);
...
std::shared_ptr<service> configuration_impl::find_service(
    service_t _service, instance_t _instance) const {
  std::lock_guard<std::mutex> its_lock(services_mutex_);
  return find_service_unlocked(_service, _instance);
}

std::shared_ptr<service> configuration_impl::find_service_unlocked(
    service_t _service, instance_t _instance) const {
...
std::shared_ptr<service> configuration_impl::find_service(
    service_t _service, const std::string &_address,
...
std::uint32_t configuration_impl::get_max_message_size_local() const {
  if (max_local_message_size_ == 0 && (VSOMEIP_MAX_LOCAL_MESSAGE_SIZE == 0 ||
  ...
  return std::uint32_t(its_max_message_size +
                       protocol::SEND_COMMAND_HEADER_SIZE);
}
...
std::uint32_t configuration_impl::get_max_message_size_unreliable() const {
  return (max_unreliable_message_size_ == 0) ? MESSAGE_SIZE_UNLIMITED
                                             : max_unreliable_message_size_;
}
...
bool configuration_impl::supports_selective_broadcasts(
    const boost::asio::ip::address &_address) const {
  return supported_selective_addresses.find(_address.to_string()) !=
         supported_selective_addresses.end();
}

bool configuration_impl::log_version() const { return log_version_; }

uint32_t configuration_impl::get_log_version_interval() const {
  return log_version_interval_;
}

bool configuration_impl::is_offered_remote(service_t _service,
                                           instance_t _instance) const {
...
bool configuration_impl::is_local_service(service_t _service,
                                          instance_t _instance) const {
...
bool configuration_impl::is_sd_enabled() const { return is_sd_enabled_; }

const std::string &configuration_impl::get_sd_multicast() const {
  return sd_multicast_;
}

uint16_t configuration_impl::get_sd_port() const { return sd_port_; }
...
ttl_t configuration_impl::get_sd_ttl() const { return sd_ttl_; }
...
std::map<std::string, std::string> configuration_impl::get_additional_data(
    const std::string &_application_name, const std::string &_plugin_name) {
  return plugins_additional_[_application_name][_plugin_name];
}

bool configuration_impl::is_e2e_enabled() const { return e2e_enabled_; }
...
void configuration_impl::load_e2e(const configuration_element &_element) {
  ...
  try {
    ...
    for (auto its_e2e = found_e2e.begin(); its_e2e != found_e2e.end();
         ++its_e2e) {
...
void configuration_impl::load_e2e_protected(
    const boost::property_tree::ptree &_tree) {
  std::string variant("");
  std::string profile("");
  ...
  e2e_configuration_[std::make_pair(service_id, event_id)] =
      std::make_shared<cfg::e2e>(variant, profile, service_id, event_id,
                                 std::move(custom_parameters));
}

std::map<e2exf::data_identifier_t, std::shared_ptr<cfg::e2e>>
configuration_impl::get_e2e_configuration() const {
  return e2e_configuration_;
}

bool configuration_impl::log_memory() const { return log_memory_; }

uint32_t configuration_impl::get_log_memory_interval() const {
  return log_memory_interval_;
}

bool configuration_impl::log_status() const { return log_status_; }
...
void configuration_impl::load_ttl_factors(
  ...
  for (const auto &i : _tree) {
    ...
    if (its_service != ILLEGAL_VALUE && its_instance != ILLEGAL_VALUE &&
        its_ttl_factor > 0) {
...
configuration_impl::get_endpoint_queue_limit(const std::string &_address,
                                             std::uint16_t _port) const {
...
void configuration_impl::load_endpoint_queue_sizes(
    const configuration_element &_element) {
  const std::string endpoint_queue_limits("endpoint-queue-limits");
  const std::string endpoint_queue_limit_external(
  ...
  try {
    ...
    if (_element.tree_.get_child_optional(endpoint_queue_limits)) {
      if (is_configured_[ET_ENDPOINT_QUEUE_LIMITS]) {
        VSOMEIP_WARNING << "Multiple definitions for " << endpoint_queue_limits
                        << " Ignoring definition from " << _element.name_;
      } else {
        ...
        for (const auto &i : _element.tree_.get_child(endpoint_queue_limits)) {
          ...
          for (const auto &j : i.second.get_child(ports)) {
            ...
            endpoint_queue_limits_[its_unicast][its_port] =
                its_queue_size_limit;
          }
        }
      }
...
void configuration_impl::load_event_debounce_ignore(
  ...
  for (auto i = _tree.begin(); i != _tree.end(); ++i) {
    ...
    if (!its_value.empty() &&
        std::find_if(its_value.begin(), its_value.end(), [](char _c) {
...
void configuration_impl::load_acceptances(
    const configuration_element &_element) {
  std::string its_acceptances_key("acceptances");
  try {
    auto its_acceptances =
        _element.tree_.get_child_optional(its_acceptances_key);
    if (its_acceptances) {
      ...
      for (auto i = its_acceptances->begin(); i != its_acceptances->end();
           ++i) {
...
void configuration_impl::load_acceptance_data(
    const boost::property_tree::ptree &_tree) {
  std::stringstream its_converter;
  try {
    ...
    std::string its_path;
    std::map<bool, std::pair<boost::icl::interval_set<std::uint16_t>,
    ...
    for (auto i = _tree.begin(); i != _tree.end(); ++i) {
      ...
      if (its_key == "address") {
              ...
              if (its_type == port_type_e::PT_OPTIONAL) {
                ...
                if (its_first != 0 && its_last != 0) {
                  its_ports.operator[](is_reliable)
                      .first.insert(boost::icl::interval<std::uint16_t>::closed(
                          its_first, its_last));
                }
              } else {
                has_secure = true;
        ...
        if (!has_optional) {
          const auto its_optional_client =
              boost::icl::interval<std::uint16_t>::closed(30491, 30499);
          const auto its_optional_client_spare =
              boost::icl::interval<std::uint16_t>::closed(30898, 30998);
          const auto its_optional_server =
              boost::icl::interval<std::uint16_t>::closed(30501, 30599);

          its_ports.operator[](is_reliable).first.insert(its_optional_client);
          its_ports.operator[](is_reliable)
              .first.insert(its_optional_client_spare);
          its_ports.operator[](is_reliable).first.insert(its_optional_server);
        }

        // If secure was not set, use default!
        if (!has_secure) {
          const auto its_secure_client =
              boost::icl::interval<std::uint16_t>::closed(32491, 32499);
          const auto its_secure_client_spare =
              boost::icl::interval<std::uint16_t>::closed(32898, 32998);
          const auto its_secure_server =
              boost::icl::interval<std::uint16_t>::closed(32501, 32599);

          its_ports.operator[](is_reliable).second.insert(its_secure_client);
          its_ports.operator[](is_reliable)
              .second.insert(its_secure_client_spare);
      ...
      if (its_method_str.size()) {
        ...
        for (const auto &j : i.second) {
          ...
          const std::uint64_t value =
              std::strtoull(j.second.data().c_str(), NULL, 10) * 1000000;
          if (key == dtime) {
        ...
        if (_is_request) {
          _service->debounce_times_requests_[its_method] = {its_debounce_time,
                                                            its_retention_time};
        } else {
        ...
        for (const auto &its_data : method.second) {
          ...
          if (!its_value.empty()) {
            if (its_data.first == "method") {
              ...
              std::uint16_t its_rest =
                  std::uint16_t(its_max_segment_length % 16);
              if (its_rest != 0) {
                VSOMEIP_WARNING << "SOMEIP/TP: max-segment-length must be "
                                   "multiple of 16. Corrected "
                                << std::dec << its_max_segment_length << " to "
                                << std::dec
                                << its_max_segment_length - its_rest;

                its_max_segment_length =
                    std::uint16_t(its_max_segment_length - its_rest);
              }
      ...
      if (its_method != 0) {
        if (_is_request) {
          ...
          if (its_entry == _service->tp_client_config_.end()) {
            ...
            VSOMEIP_WARNING
                << "SOME/IP-TP: Multiple client configurations for method ["
                << std::hex << std::setfill('0') << std::setw(4)
                << _service->service_ << "." << std::setw(4)
                << _service->instance_ << "." << std::setw(4) << its_method
                << "]:"
                << " using (" << std::dec << its_entry->second.first << ", "
          ...
          if (its_entry == _service->tp_service_config_.end()) {
            ...
            VSOMEIP_WARNING
                << "SOME/IP-TP: Multiple service configurations for method ["
                << std::hex << std::setfill('0') << std::setw(4)
                << _service->service_ << "." << std::setw(4)
                << _service->instance_ << "." << std::setw(4) << its_method
                << "]:"
        ...
        VSOMEIP_ERROR << "SOME/IP-TP configuration contains invalid entry. No "
...
void configuration_impl::load_udp_receive_buffer_size(
    const configuration_element &_element) {
  ...
  try {
    if (_element.tree_.get_child_optional(its_buffer_size)) {
      if (is_configured_[ET_UDP_RECEIVE_BUFFER_SIZE]) {
        ...
        const std::string its_data(
            _element.tree_.get_child(its_buffer_size).data());
        try {
          udp_receive_buffer_size_ = std::stoi(its_data.c_str(), nullptr, 10);
        } catch (const std::exception &e) {
          VSOMEIP_ERROR << __func__ << ": " << its_buffer_size << " "
                        << e.what();
        }
...
void configuration_impl::load_secure_services(
    const configuration_element &_element) {
...
void configuration_impl::load_secure_service(
    const boost::property_tree::ptree &_tree) {
  try {
    service_t its_service(0);
    instance_t its_instance(0);
...
std::shared_ptr<debounce_filter_impl_t> configuration_impl::get_debounce(
    const std::string &_name, service_t _service, instance_t _instance,
    event_t _event) const {
...
void configuration_impl::load_tcp_restart_settings(
    const configuration_element &_element) {
  ...
  try {
    if (_element.tree_.get_child_optional(tcp_restart_aborts_max)) {
      if (is_configured_[ET_TCP_RESTART_ABORTS_MAX]) {
        VSOMEIP_WARNING << "Multiple definitions for " << tcp_restart_aborts_max
                        << " Ignoring definition from " << _element.name_;
      } else {
    ...
    if (_element.tree_.get_child_optional(tcp_connect_time_max)) {
      if (is_configured_[ET_TCP_CONNECT_TIME_MAX]) {
        VSOMEIP_WARNING << "Multiple definitions for " << tcp_connect_time_max
                        << " Ignoring definition from " << _element.name_;
      } else {
...
bool configuration_impl::is_protected_port(
  ...
  if (found_address != sd_acceptance_rules_.end()) {
    ...
    if (found_reliability != found_address->second.second.end()) {
      ...
      bool is_optional = (found_reliability->second.first.find(its_range) !=
                          found_reliability->second.first.end());

      bool is_secure = (found_reliability->second.second.find(its_range) !=
...
void configuration_impl::set_sd_acceptance_rule(
    const boost::asio::ip::address &_address, port_range_t _port_range,
    port_type_e _type, const std::string &_path, bool _reliable, bool _enable,
    bool _default) {
  (void)_port_range;
  (void)_type;
  ...
  if (found_address != sd_acceptance_rules_.end()) {
    if (found_address->second.first.length() > 0 &&
      ...
      VSOMEIP_WARNING << __func__ << ": activation path for IP: " << _address
                      << " differ: " << found_address->second.first << " vs. "
                      << _path << " will use: " << found_address->second.first;
    } else {
      found_address->second.first = _path;
    }
    const auto found_reliability = found_address->second.second.find(_reliable);
    if (found_reliability != found_address->second.second.end()) {
      if (_enable) {
        ...
        // configured
        if (!_default || (found_reliability->second.first.empty() &&
    ...
    sd_acceptance_rules_.emplace(std::make_pair(
        ...
        std::make_pair(
            ...
            std::map<bool, std::pair<boost::icl::interval_set<std::uint16_t>,
                                     boost::icl::interval_set<std::uint16_t>>>(
                {{_reliable, std::make_pair(its_optional_default,
                                            its_secure_default)}}))));
    if (!rules_active) {
      sd_acceptance_rules_active_.insert(_address);
    }
...
configuration::sd_acceptance_rules_t
configuration_impl::get_sd_acceptance_rules() {
  std::lock_guard<std::mutex> its_lock(sd_acceptance_required_ips_mutex_);
...
bool configuration_impl::is_secure_service(service_t _service,
                                           instance_t _instance) const {
  std::lock_guard<std::mutex> its_lock(secure_services_mutex_);
...
bool configuration_impl::is_tp_client(service_t _service,
                                      const std::string &_address,
                                      std::uint16_t _port,
                                      method_t _method) const {
  bool ret(false);

  const auto its_service = find_service(_service, _address, _port);
...
bool configuration_impl::is_tp_service(service_t _service,
                                       const std::string &_address,
                                       std::uint16_t _port,
                                       method_t _method) const {
  bool ret(false);
...
bool configuration_impl::log_statistics() const { return log_statistics_; }
...
partition_id_t configuration_impl::get_partition_id(
    service_t _service, instance_t _instance) const {
...
reliability_type_e configuration_impl::get_reliability_type(
    const boost::asio::ip::address &_reliable_address,
    const uint16_t &_reliable_port,
    const boost::asio::ip::address &_unreliable_address,
    const uint16_t &_unreliable_port) const {
  if (_reliable_port != ILLEGAL_PORT && _unreliable_port != ILLEGAL_PORT &&
