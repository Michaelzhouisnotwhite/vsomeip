...
local_uds_server_endpoint_impl::local_uds_server_endpoint_impl(
    ...
    : local_uds_server_endpoint_base_impl(
          _endpoint_host, _routing_host, _local, _io,
          _configuration->get_max_message_size_local(),
          _configuration->get_endpoint_queue_limit_local(), _configuration),
      acceptor_(_io),
  ...
  if (ec) VSOMEIP_ERROR << __func__ << ": open failed (" << ec.message() << ")";

  acceptor_.set_option(boost::asio::socket_base::reuse_address(true), ec);
  if (ec)
    VSOMEIP_ERROR << __func__ << ": set reuse address option failed ("
                  << ec.message() << ")";

  acceptor_.bind(_local, ec);
  if (ec) VSOMEIP_ERROR << __func__ << ": bind failed (" << ec.message() << ")";

  acceptor_.listen(boost::asio::socket_base::max_connections, ec);
  if (ec)
    VSOMEIP_ERROR << __func__ << ": listen failed (" << ec.message() << ")";
#ifndef __QNX__
...
local_uds_server_endpoint_impl::local_uds_server_endpoint_impl(
    ...
    const std::shared_ptr<routing_host> &_routing_host,
    const endpoint_type &_local, boost::asio::io_context &_io,
    int native_socket, const std::shared_ptr<configuration> &_configuration,
    bool _is_routing_endpoint)
    : local_uds_server_endpoint_base_impl(
          _endpoint_host, _routing_host, _local, _io,
          _configuration->get_max_message_size_local(),
  ...
  if (ec)
    VSOMEIP_ERROR << __func__ << ": assign failed (" << ec.message() << ")";
...
bool local_uds_server_endpoint_impl::is_local() const { return true; }

/**
 */
void local_uds_server_endpoint_impl::start() {
  // all connections request_service->create_local_server->start
  ...
  if (acceptor_.is_open()) {
...

    {
      ...
      acceptor_.async_accept(
          ...
          [self = std::dynamic_pointer_cast<local_uds_server_endpoint_impl>(
               shared_from_this()),
           new_connection](boost::system::error_code ec) {
            self->accept_cbk(new_connection, ec);
          });
    }
  }
}
...
bool local_uds_server_endpoint_impl::send(const uint8_t *_data,
                                          uint32_t _size) {
#if 0
  ...
  client_t its_client;
  std::memcpy(&its_client, &_data[protocol::COMMAND_HEADER_SIZE],
              sizeof(its_client));
...
bool local_uds_server_endpoint_impl::get_default_target(
    service_t, local_uds_server_endpoint_impl::endpoint_type &) const {
  return false;
}

bool local_uds_server_endpoint_impl::add_connection(
    const client_t &_client, const std::shared_ptr<connection> &_connection) {
  bool ret = false;
  std::lock_guard<std::mutex> its_lock(connections_mutex_);
...
void local_uds_server_endpoint_impl::accept_cbk(
    const connection::ptr &_connection,
  ...
  if (!_error) {
    ...
    if (auto creds =
            credentials::receive_credentials(its_socket.native_handle())) {
      ...
      VSOMEIP_WARNING << "vSomeIP Security: Client 0x" << std::hex
                      << its_host->get_client()
                      << " is rejecting new connection because client "
                         "credentials couldn't be received!";
      boost::system::error_code er;
      its_socket.shutdown(socket_type::shutdown_both, er);
      its_socket.close(er);
      return;
    }

    if (its_host && configuration_->is_security_enabled()) {
      if (!configuration_->check_routing_credentials(its_client,
                                                     &its_sec_client)) {
        VSOMEIP_WARNING << "vSomeIP Security: Rejecting new connection with "
                           "routing manager client ID 0x"
                        << std::hex << its_client << " uid/gid= " << std::dec
                        << its_sec_client.user << "/" << its_sec_client.group
                        << " because passed credentials do not match with "
                           "routing manager credentials!";
        boost::system::error_code er;
      ...
      if (is_routing_endpoint_) {
      ...
      } else {
        {
          ...
          if (found_client != connections_.end()) {
            VSOMEIP_WARNING << "vSomeIP Security: Client 0x" << std::hex
                            ...
                            << " because of already existing connection using "
                               "same client ID";
            boost::system::error_code er;
            its_socket.shutdown(its_socket.shutdown_both, er);
            its_socket.close(er);
        ...
        if (!policy_manager_impl::get()->check_credentials(its_client,
                                                           &its_sec_client)) {
          VSOMEIP_WARNING << "vSomeIP Security: Client 0x" << std::hex
                          << its_host->get_client()
                          << " received client credentials from client 0x"
                          << its_client
      ...
      policy_manager_impl::get()->store_client_to_sec_client_mapping(
          its_client, &its_sec_client);
      policy_manager_impl::get()->store_sec_client_to_client_mapping(
          &its_sec_client, its_client);
...
local_uds_server_endpoint_impl::connection::create(
  ...
  const std::uint32_t its_initial_buffer_size = static_cast<std::uint32_t>(
      protocol::COMMAND_HEADER_SIZE + sizeof(instance_t) + sizeof(bool) +
      sizeof(bool));
...
void local_uds_server_endpoint_impl::connection::start() {
  ...
  if (socket_.is_open()) {
    ...
    if (recv_buffer_size_ > its_capacity) {
      VSOMEIP_ERROR
          << __func__
          << "Received buffer size is greater than the buffer capacity!"
    ...
    try {
      if (missing_capacity_) {
        if (missing_capacity_ > MESSAGE_SIZE_UNLIMITED) {
          VSOMEIP_ERROR
              << "Missing receive buffer capacity exceeds allowed maximum!";
          return;
        }
        const std::size_t its_required_capacity(recv_buffer_size_ +
                                                missing_capacity_);
        ...
        // And set buffer_size to recv_buffer_size_initial_, the same of our
        // resize
    ...
    auto its_storage = std::make_shared<local_endpoint_receive_op::storage>(
        socket_,
        std::bind(&local_uds_server_endpoint_impl::connection::receive_cbk,
                  shared_from_this(), std::placeholders::_1,
                  std::placeholders::_2, std::placeholders::_3,
                  std::placeholders::_4),
        &recv_buffer_[recv_buffer_size_], left_buffer_size,
        std::numeric_limits<std::uint32_t>::max(),
        std::numeric_limits<std::uint32_t>::max(),
        std::numeric_limits<std::size_t>::min());

    socket_.async_wait(socket_type::wait_read,
                       local_endpoint_receive_op::receive_cb(its_storage));
#else
...
void local_uds_server_endpoint_impl::connection::stop() {
  ...
  if (socket_.is_open()) {
    if (-1 == fcntl(socket_.native_handle(), F_GETFD)) {
      VSOMEIP_ERROR << "lse: socket/handle closed already '"
...
void local_uds_server_endpoint_impl::connection::send_queued(
...

  {
    ...
    boost::asio::async_write(
        socket_, bufs,
        std::bind(&local_uds_server_endpoint_impl::connection::send_cbk,
                  shared_from_this(), _buffer, std::placeholders::_1,
                  std::placeholders::_2));
  }
}

client_t local_uds_server_endpoint_impl::assign_client(const byte_t *_data,
                                                       uint32_t _size) {
  ...
  return utility::request_client_id(configuration_, its_command.get_name(),
...
void local_uds_server_endpoint_impl::get_configured_times_from_endpoint(
  ...
  VSOMEIP_ERROR
      << "local_uds_server_endpoint_impl::get_configured_times_from_endpoint.";
}

void local_uds_server_endpoint_impl::connection::send_cbk(
    const message_buffer_ptr_t _buffer, boost::system::error_code const &_error,
    std::size_t _bytes) {
  (void)_buffer;
  (void)_bytes;
  if (_error)
...
void local_uds_server_endpoint_impl::connection::receive_cbk(
  ...
  std::shared_ptr<vsomeip_v3::configuration> its_config =
      its_server->configuration_;
  ...
  if (!_error && 0 < _bytes) {
    ...
    if (recv_buffer_size_ + _bytes < recv_buffer_size_) {
      VSOMEIP_ERROR
          << "receive buffer overflow in local server endpoint ~> abort!";
      return;
    }
    ...
    do {
      ...
      if (!message_is_empty) {
        if (its_start + protocol::COMMAND_POSITION_SIZE + 3 <
          ...
          its_end = its_start + protocol::COMMAND_POSITION_SIZE + 3 +
                    its_command_size;
        } else {
          its_end = its_start;
        }
        ...
        if (recv_buffer_size_ + its_iteration_gap < its_end + 4 ||
          ...
          // If need to change the recv_buffer_, change the value of
          // missing_capacity_ in this if/else, otherwise it is 0
          if (its_start - its_iteration_gap + its_command_size +
                  protocol::COMMAND_HEADER_SIZE + protocol::TAG_SIZE >
              recv_buffer_size_) {
            missing_capacity_ = std::uint32_t(its_start) -
                                std::uint32_t(its_iteration_gap) +
                                its_command_size +
                                std::uint32_t(protocol::COMMAND_HEADER_SIZE +
                                              protocol::TAG_SIZE) -
                                std::uint32_t(recv_buffer_size_);
          } else if (recv_buffer_size_ <
                     protocol::COMMAND_HEADER_SIZE + protocol::TAG_SIZE) {
            // to little data to read out the command size
            // minimal amount of data needed to read out command size = header +
            // tag size
      ...
      if (!message_is_empty &&
        ...
        if (its_server->is_routing_endpoint_ &&
            recv_buffer_[its_start] ==
                byte_t(protocol::id_e::ASSIGN_CLIENT_ID)) {
          ...
          if (its_config && its_config->is_security_enabled()) {
            ...
            if (!its_server->add_connection(its_client, shared_from_this())) {
              VSOMEIP_WARNING
                  ...
                  << " is rejecting new connection with client ID 0x"
                  << its_client << " uid/gid= " << std::dec << sec_client_.user
                  << "/" << sec_client_.group
              ...
              VSOMEIP_WARNING
                  ...
                  << " which violates the security policy : uid/gid="
                  << std::dec << sec_client_.user << "/" << sec_client_.group;
        ...
        if (its_iteration_gap) {
          ...
          if (missing_capacity_ &&
              missing_capacity_ <=
                  recv_buffer_.capacity() - recv_buffer_size_) {
            missing_capacity_ = 0;
          }
  ...
  if (is_stopped_ || _error == boost::asio::error::eof ||
    ...
    policy_manager_impl::get()->remove_client_to_sec_client_mapping(
        bound_client_);
...
void local_uds_server_endpoint_impl::connection::set_bound_client(
    client_t _client) {
...
std::string local_uds_server_endpoint_impl::connection::get_bound_client_host()
    const {
...
std::string local_uds_server_endpoint_impl::connection::get_path_remote()
    const {
  boost::system::error_code ec;
...
void local_uds_server_endpoint_impl::print_status() {
  ...
  VSOMEIP_INFO << "status lse: " << its_local_path
...
bool local_uds_server_endpoint_impl::is_reliable() const { return false; }
...
void local_uds_server_endpoint_impl::send_client_identifier(
    const client_t &_client) {
