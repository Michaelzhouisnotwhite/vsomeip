...
typedef std::function<void(boost::system::error_code const &_error,
                           size_t _size, const std::uint32_t &,
                           const std::uint32_t &)>
    receive_handler_t;
...
struct storage : public std::enable_shared_from_this<storage> {
  ...
  storage(socket_type_t &_socket, receive_handler_t _handler, byte_t *_buffer,
          size_t _length, uid_t _uid, gid_t _gid, size_t _bytes)
      : socket_(_socket),
        handler_(_handler),
        buffer_(_buffer),
        length_(_length),
...
inline std::function<void(boost::system::error_code _error)> receive_cb(
    std::shared_ptr<storage> _data) {
  return [_data](boost::system::error_code _error) {
    if (!_error) {
      ...
      for (;;) {
        ...
        errno = 0;
        its_result =
            ::recvmsg(_data->socket_.native_handle(), &its_header, its_flags);
        _error = boost::system::error_code(
            its_result < 0 ? errno : 0,
            boost::asio::error::get_system_category());
        _data->bytes_ += _error ? 0 : static_cast<size_t>(its_result);

        if (_error == boost::asio::error::interrupted) continue;

        if (_error == boost::asio::error::would_block ||
            _error == boost::asio::error::try_again) {
          _data->socket_.async_wait(socket_type_t::wait_read,
                                    receive_cb(_data));
          return;
        }
        ...
        if (_data->bytes_ == 0) _error = boost::asio::error::eof;

        // Extract credentials (UID/GID)
        struct ucred *its_credentials;
        for (struct cmsghdr *cmsg = CMSG_FIRSTHDR(&its_header); cmsg != NULL;
